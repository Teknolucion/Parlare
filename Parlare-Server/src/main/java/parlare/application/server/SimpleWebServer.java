...
public class SimpleWebServer extends ParlareHTTPD {
    /**
    ...
    private static final String LICENCE =
        "Copyright (c) 2012-2013 by Paul S. Hawke, 2001,2005-2013 by Jarno Elonen, 2010 by Konstantinos Togias, 2013 by Jesus Rodriguez Miranda\n"
            + "\n"
    ...
    public static void main(String[] args) throws KeyStoreException {
        ...
        int portSSL = Integer.parseInt(settings.getString("portSSL"));  
        String hostSSL = settings.getString("hostSSL");
        ...
        for (int i = 0; i < args.length; ++i) {
            ...
            } else if (args[i].equalsIgnoreCase("-h") || args[i].equalsIgnoreCase("--host-ssl")) {
                hostSSL = args[i + 1];
            } else if (args[i].equalsIgnoreCase("-p") || args[i].equalsIgnoreCase("--port-ssl")) {
                portSSL = Integer.parseInt(args[i + 1]);
            } else if (args[i].equalsIgnoreCase("-s") || args[i].equalsIgnoreCase("--ssl")) {
                ssl = true;
        ...
        for (WebServerPluginInfo info : serviceLoader) {
            ...
            for (String mime : mimeTypes) {
                ...
                if (!quiet) {
                    System.out.println("# Found plugin for Mime type: \"" + mime + "\"");
                    if (indexFiles != null) {
        ...
        if ( ssl == true ) 
            ServerRunner.executeInstanceSSL(new SimpleWebServer(hostSSL, portSSL, rootDirs, quiet));
...
//    }
    ...
    public void setCookie(CookieHandler cookies) {
        for (String cookieName : cookies) {
            cookies.set(cookieName, cookies.read(cookieName), 30);
        }
    }
    ...
    public Response serve(HTTPSession session) {
        ...
        Map<String, String> files = header;
        ...
        if (!quiet) {
            ...
            System.out.println(calendar.getTime() + " " + session.getMethod() + " '" + uri + "' ");

            Iterator<String> e = header.keySet().iterator();
            while (e.hasNext()) {
                String value = e.next();
                System.out.println("  HEADER: '" + value + "' = '" + header.get(value) + "'");
            }
            e = parms.keySet().iterator();
            while (e.hasNext()) {
                String value = e.next();
                System.out.println("  PARAM: '" + value + "' = '" + parms.get(value) + "'");
            }
            e = cookies.iterator();
            while (e.hasNext()) {
                String value = e.next();
                System.out.println("  COOKIES: '" + value + "' = '" + cookies.read(value) + "'");
            }
        ...
        for (File homeDir : getRootDirs()) {
            ...
            if (!homeDir.isDirectory()) {
                return createResponse(Response.Status.INTERNAL_ERROR, ParlareHTTPD.MIME_PLAINTEXT,
    ...
    private Response respond(Map<String, String> headers, Map<String, String> params, String uri) {
        ...
        if (uri.startsWith("/src/main/") || uri.endsWith("/src/main/") || uri.contains("../")) {
            return createResponse(Response.Status.FORBIDDEN, ParlareHTTPD.MIME_PLAINTEXT, "FORBIDDEN: Won't serve ../ for security reasons.");
        }
        ...
        if (!canServeUri) {
            return createResponse(Response.Status.NOT_FOUND, ParlareHTTPD.MIME_PLAINTEXT, "Error 404, file not found.");
        }
        ...
        if (f.isDirectory() && !uri.endsWith("/")) {
            uri += "/";
            Response res = createResponse(Response.Status.REDIRECT, ParlareHTTPD.MIME_HTML, "<html><body>Redirected: <a href=\"" +
                uri + "\">" + uri + "</a></body></html>");
        ...
        if (f.isDirectory()) {
            ...
            if (indexFile == null) {
                if (f.canRead()) {
                    ...
                    return createResponse(Response.Status.OK, ParlareHTTPD.MIME_HTML, listDirectory(uri, f));
                } else {
                    return createResponse(Response.Status.FORBIDDEN, ParlareHTTPD.MIME_PLAINTEXT, "FORBIDDEN: No directory listing.");
                }
            } else {
                return respond(headers, params, uri + indexFile);
            }
        }
        ...
        if (plugin != null) {
            ...
            response = plugin.serveFile(uri, headers, params, f, mimeTypeForFile);
            
            System.out.println(response);

            if (response != null && response instanceof InternalRewrite) {
                InternalRewrite rewrite = (InternalRewrite) response;
                return respond(rewrite.getHeaders(), params, rewrite.getUri());
            }
        } else {
            response = serveFile(uri, headers, params, f, mimeTypeForFile);
        }
        
        return response != null ? response :
            createResponse(Response.Status.NOT_FOUND, ParlareHTTPD.MIME_PLAINTEXT, "Error 404, file not found.");
    }
     ...
     */
    Response serveFile(String uri, Map<String, String> header, Map<String, String> params, File file, String mime) {
        Response res;
        try {
            // Calculate etag
            ...
            if (range != null && startFrom >= 0) {
                if (startFrom >= fileLen) {
                    res = createResponse(Response.Status.RANGE_NOT_SATISFIABLE, ParlareHTTPD.MIME_PLAINTEXT, "");
            ...
            res = createResponse(Response.Status.FORBIDDEN, ParlareHTTPD.MIME_PLAINTEXT, "FORBIDDEN: Reading file failed.");
        }
